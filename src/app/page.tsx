...
import {
  ...
  BookOpen, Plus, MessageSquare, FileDown, Menu, X, ChevronRight,
...
const GoogleLoginButton = ({ onSuccess }: { onSuccess: (token: string) => void }) => {
  ...
  return (
    <button onClick={() => login()} className="w-full bg-white dark:bg-[#0d1117] border-2 border-gray-200 dark:border-gray-800 hover:border-blue-500 p-4 rounded-2xl font-bold text-gray-700 dark:text-gray-300 flex items-center justify-center gap-3 transition active:scale-95 shadow-sm">
...
export default function Home() {
  ...
  const [writingMode, setWritingMode] = useState('Conversational');
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    } catch (err) { alert("Server error - check CORS or backend status"); } finally { setLoading(false); }
  };
  ...
  const handleAuth = async () => { /* Standard Email Auth Logic */ };
  const handleGoogleAuth = async (accessToken: string) => { /* Google Auth Logic */ };
  ...
  return (
    <div className={`flex h-screen ${themeClasses.bg} ${themeClasses.text} font-sans transition-colors duration-300 overflow-hidden`}>
      ...
      {/* SIDEBAR */}
      <aside className={`fixed md:relative z-50 inset-y-0 left-0 w-72 md:w-80 ${themeClasses.sidebar} border-r ${themeClasses.border} flex flex-col p-6 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} ${zenMode ? 'md:hidden' : 'flex'}`}>
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="md:hidden p-2 text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-lg">
            <X size={20} />
          </button>
        </div>
          ...
          {sessions.map((s, i) => (
            <div key={i} className="group relative">
              <button
                ...
                className={`flex items-center gap-3 p-3 w-full rounded-xl text-left text-sm transition font-medium ${currentSessionId === s.sessionId ? 'bg-blue-50 text-blue-600 border border-blue-100 dark:bg-blue-900/20 dark:border-blue-800' : `hover:bg-black/5 dark:hover:bg-white/5 ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}`}
              >
      ...
      <div className={`flex-1 flex relative w-full ${isDraftOpen ? 'hidden md:flex' : 'flex'}`}>
        <div className="flex flex-col w-full transition-all duration-500 h-full">
          <header className={`p-4 md:p-5 border-b ${themeClasses.border} flex justify-between items-center ${themeClasses.bg} z-10 shadow-sm shadow-black/5`}>
            <div className="flex items-center gap-2 md:gap-4">
                ...
                <select
                    ...
                    className={`bg-transparent border ${themeClasses.border} text-[10px] md:text-xs font-bold uppercase rounded-lg px-2 py-1.5 md:px-3 outline-none cursor-pointer focus:border-blue-500 text-gray-500 max-w-[120px] md:max-w-none truncate`}
                >
                    <option className="text-black">Conversational</option>
                    <option className="text-black">Professional</option>
                    <option className="text-black">Dramatic</option>
                </select>
            </div>
            <button onClick={() => setIsDraftOpen(!isDraftOpen)} className={`flex items-center gap-2 p-2 px-3 md:p-2.5 rounded-xl transition font-bold text-xs uppercase tracking-wider ${isDraftOpen ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
              <LayoutPanelLeft size={18} />
              <span className="hidden sm:block">{isDraftOpen ? 'Close Draft' : 'View Draft'}</span>
            </button>
          </header>

          <main className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 space-y-6 md:space-y-8 scroll-smooth pb-28 md:pb-0">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center opacity-40 text-center select-none px-4">
                <div className="bg-blue-50 dark:bg-blue-900/20 p-5 md:p-6 rounded-full mb-4 md:mb-6">
                    <BookOpen size={48} className="text-blue-500 md:w-16 md:h-16" />
                </div>
                <h2 className="text-2xl md:text-3xl italic font-serif text-gray-800 dark:text-gray-300">"Let's write your story..."</h2>
                <p className="mt-2 text-xs md:text-sm font-sans uppercase tracking-widest text-gray-500">I will interview you and write the book.</p>
              </div>
            )}
            
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`p-4 md:p-6 rounded-3xl max-w-[90%] md:max-w-[85%] leading-relaxed text-sm md:text-[1.1rem] transition-all relative group ${
                  m.role === 'user' ? 'bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-br-sm shadow-md shadow-blue-500/20' : `${themeClasses.chatAi} rounded-tl-sm text-gray-700 dark:text-gray-300`
                }`}>
                  ...
                  {m.role === 'ai' && (
                    <button
                      ...
                      className="mt-3 md:mt-4 flex items-center gap-1.5 md:gap-2 text-[10px] font-bold text-gray-400 hover:text-blue-500 transition uppercase tracking-widest border border-gray-200 dark:border-gray-700 px-2.5 py-1 md:px-3 md:py-1.5 rounded-full hover:bg-blue-50 dark:hover:bg-gray-800"
                    >
                      {speakingIndex === i ? (
                        <><VolumeX size={12} className="text-red-500"/> Stop</>
                      ) : (
                        <><Volume2 size={12} className="text-blue-500"/> Listen</>
                      )}
                    </button>
                  )}
                </div>
          ...
          <footer className={`absolute md:relative bottom-0 w-full p-3 md:p-6 lg:px-12 ${themeClasses.bg} border-t ${themeClasses.border} shadow-[0_-10px_40px_rgba(0,0,0,0.05)] md:shadow-none z-20`}>
            
            {!user && messages.length > 0 && (
              <div className="hidden md:flex bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-900/30 text-yellow-800 dark:text-yellow-500 text-xs md:text-sm font-bold px-4 py-3 rounded-2xl mb-4 justify-between items-center shadow-sm">
                <span className="flex items-center gap-2"><AlertTriangle size={16} /> Chatting as a Guest. Login to save!</span>
                <button onClick={() => setShowAuth(true)} className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-xl transition uppercase tracking-wider text-[10px]">Login</button>
              </div>
            )}

            {messages.length > 0 && (
                <div className="flex gap-2 mb-3 overflow-x-auto hide-scrollbar custom-scrollbar">
                    <button onClick={handleDigDeeper} className="shrink-0 flex items-center gap-1.5 md:gap-2 bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-600 text-gray-500 text-[10px] md:text-xs font-bold uppercase px-4 md:px-5 py-2 md:py-2.5 rounded-full transition-all border shadow-sm border-gray-200 dark:border-gray-700">
                        <Telescope size={14}/> Ask to Dig Deeper
                    </button>
                </div>
            )}
            ...
            <div className="relative flex items-center gap-2 md:gap-4">
              <button
                ...
                className={`p-3 md:p-4 rounded-xl md:rounded-2xl transition-all border ${themeClasses.border} shadow-sm shrink-0 ${isListening ? 'bg-red-50 border-red-200 text-red-500 animate-pulse' : 'bg-white dark:bg-[#161b22] text-gray-400 hover:text-blue-500 hover:border-blue-200'}`}
              >
      ...
      {/* DRAFT / MANUSCRIPT PANEL - MOBILE FULL SCREEN */}
      <div className={`${isDraftOpen ? 'flex' : 'hidden'} fixed inset-0 z-[60] md:relative md:flex flex-col w-full md:w-1/2 lg:w-[45%] bg-white dark:bg-[#0d1117] md:border-l ${themeClasses.border} shadow-2xl animate-in slide-in-from-right duration-300`}>
        <header className={`p-4 md:p-5 border-b ${themeClasses.border} flex justify-between items-center bg-[#fcfcfc] dark:bg-[#161b22]`}>
          <div className="flex items-center gap-2 md:gap-3">
            {/* Mobile Back Button */}
            <button onClick={() => setIsDraftOpen(false)} className="md:hidden p-1.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-200">
                <ChevronRight size={20}/>
            </button>
            <div className="bg-blue-100 dark:bg-blue-900/30 p-1.5 rounded-lg hidden sm:block">
                <PenLine size={18} className="text-blue-600"/>
            </div>
            <h2 className="font-bold text-xs md:text-sm text-gray-800 dark:text-gray-200 uppercase tracking-widest">Manuscript</h2>
          </div>
        ...
        <div className="flex-1 p-6 md:p-12 lg:p-16 overflow-y-auto custom-scrollbar">
            <textarea
            className={`w-full h-full bg-transparent outline-none resize-none font-serif text-[1.1rem] md:text-[1.35rem] leading-[1.8] md:leading-[2.2] transition-all text-gray-800 dark:text-gray-300 placeholder:text-gray-300 dark:placeholder:text-gray-700`}
      ...
      {showAuth && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[100] backdrop-blur-sm transition-all">
           ...
           <button onClick={() => setShowAuth(false)} className="absolute top-5 right-5 text-gray-400 hover:text-red-500">
               <X size={24}/>
           </button>
           <h2 className="text-3xl md:text-4xl font-black mb-6 md:mb-8 text-center tracking-tighter italic uppercase text-blue-600">Join Muse</h2>
